...
create table T_ACCOUNT (ID integer identity primary key, NUMBER varchar(9), NAME varchar(50) not null, EMAIL varchar(80), unique(NUMBER));
...
create table T_REWARD (ID integer identity primary key, CONFIRMATION_NUMBER varchar(25) not null,DINING_TRANSACTION_ID varchar(25), REWARD_AMOUNT double not null, REWARD_DATE date not null, ACCOUNT_NUMBER varchar(9) not null, DINING_AMOUNT double not null, DINING_MERCHANT_NUMBER varchar(10) not null, DINING_DATE date not null, unique(CONFIRMATION_NUMBER), unique(DINING_TRANSACTION_ID));
